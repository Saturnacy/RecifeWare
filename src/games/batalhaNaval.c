...
void batalhaNaval()
{
    ...
    while (timer.fase == FASE_PAVIO && destruidos < META_DESTRUIDOS && !WindowShouldClose())
    {
        ...
        if (canAngle < -80.0f * DEG2RAD)
            canAngle = -80.0f * DEG2RAD;
        if (canAngle > -5.0f * DEG2RAD)
            canAngle = -5.0f * DEG2RAD;
        ...
        for (int i = 0; i < MAX_BALAS; i++)
        {
            ...
            if (balas[i].x > screenWidth + 20 || balas[i].x < -20 || balas[i].y < -20 || balas[i].y > screenHeight + 20)
                balas[i].ativo = 0;
        }
