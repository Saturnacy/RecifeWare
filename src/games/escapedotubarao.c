...
void escapeDoTubarao(int dificuldade) {
    ...
    while (timer.fase==FASE_PAVIO && !colisao && !WindowShouldClose()) {
        ...
        if (playerX+playerRaio>arenaX+arenaSizeW) playerX=arenaX+arenaSizeW-playerRaio;
        if (playerY-playerRaio<arenaY) playerY=arenaY+playerRaio;
        if (playerY+playerRaio>arenaY+arenaSizeH) playerY=arenaY+arenaSizeH-playerRaio;
        float dx=playerX-tubaraoX;
        float dy=playerY-tubaraoY;
        float dist=sqrtf(dx*dx+dy*dy);
        if (dist>0) { tubaraoX+=(dx/dist)*tubaraoVel; tubaraoY+=(dy/dist)*tubaraoVel; }
        if (CheckCollisionCircles((Vector2){playerX,playerY},playerRaio,(Vector2){tubaraoX,tubaraoY},tubaraoRaio)) colisao=1;
        BeginDrawing();
