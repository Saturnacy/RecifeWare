...
int main()
{
    ...
    while (!WindowShouldClose() && !exitGame)
    {
        ...
        case STATE_SPLASH_FADE_IN:
            fadeAlpha -= 8.0f;
            if (fadeAlpha <= 0) { fadeAlpha = 0; currentState = STATE_SPLASH_CESAR; }
            break;

        case STATE_SPLASH_CESAR:
            frameCounter++;
            if (frameCounter > 120) { currentState = STATE_FADE_OUT; frameCounter = 0; }
            break;
        ...
        case STATE_TITLE_ANIM:
            ...
            if (lsPhase == LS_ENTERING)
            {
                ...
                if (lsX >= LS_TARGET_X - 1.0f) { lsX = LS_TARGET_X; lsPhase = LS_LOADING; }
            }
            else if (lsPhase == LS_LOADING)
            {
                effectTimer += dt;
                lsProgress  += dt / 5.0f;
                if (lsProgress >= 1.0f) { lsProgress = 1.0f; lsPhase = LS_EXITING; }
            }
        ...
        case STATE_GAMEPLAY:
            ...
            else
            {
                ...
                switch (gid)
                {
                case 1: cozinhaDeTapioca();      break;
                case 2: crabber();               break;
                case 3: escaladaBrenan();        break;
                case 4: escapeDoTubarao(1);      break;
                case 5: mulaSemCabeca();         break;
                case 6: pegueAsCapivaras();      break;
                }
        ...
        case STATE_RANKING:
        {
            ...
            for (int r = 0; r < 10; r++)
            {
                ...
                Color numColor = (r == 0) ? GOLD : (r == 1) ? LIGHTGRAY : (r == 2) ? ORANGE : WHITE;
                char pos_str[8];
        ...
        if (currentState == STATE_TITLE_ANIM)
        {
            ...
            {
                int idx = leftOrder[i];
                if (currentState == STATE_TITLE_ANIM && lsProgress < (i + 1) / 6.0f) break;
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (currentState == STATE_TITLE_ANIM && lsProgress < (i + 5) / 6.0f) break;
                iw = icons[idx].width  * ICON_SCALE;
                ih = icons[idx].height * ICON_SCALE;
